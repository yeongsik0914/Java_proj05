...
abstract class Animal {	//클래스 내부에 추상 메소드가 단 하나라도 존재하면, 추상 클래스가 되어야 한다.
	...
	//추상 메소드 : 실행블락이 존재하지 않는 메소드
		//반드시 자식 클래스에서 오버라이딩 하여 구현부를 만들어야 한다.
		//프로젝트를 설계할 때 메소드 이름을 다른 이름으로 사용하지 못하도록 할 수 있다.
		//부모 클래스에서는 메소드 이름을 선언만 해주고, 자식 클래스에서 구현하도록 함.
		// 자식 클래스에서는 메소드 이름을 변경할 수 없다.
	abstract void run ();	//구현부가 없는 선언만 된 메소드 : 추상메소드 {} 없음
}

class Cat extends Animal { //콘크리트 클래스, 부모의 추상 메소드를 모두 구현한 클래스
	//부모 클래스에서 추상 메소드가 존재할 경우 자식에서는 추상 메소드를 오버라이딩 하여 구현해야 한다.
