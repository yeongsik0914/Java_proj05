...
interface A {	// 인터페이스 이름 : 첫 자는 반드시 대문자.	//정식 표현
...
class Bb implements B {
	...
	@Override
	public void abc() {	//오류 발생
		System.out.println("B 인터페이스를 구현한 메소드");
	}
}


//추상 클래스 : 추상 메소드를 포함한 클래스, 추상 클래스내의 추상 메소드는 abstract 키를 생략하면 안된다.
abstract class C {
...
class Cc extends C {
	@Override
	void abc() {
		System.out.println("추상 클래스 C를 구현한 메소드 : Cc 클래스의 오버라이딩 된 메소드");
	}
}
...
public class Interface01 {
	...
	public static void main(String[] args) {
		/*
		 	인터페이스 (Interface) : 내부와 외부를 연결해주는 접점, 콘센트
		 		- 객체 생성이 불가함. 타입으로 지정은 가능
		 		- 모든 필드는 public static final이 생략되어 있다.
		 			: 값을 수정할 수 없다.
		 		- 모든 메소드는 public abstract이 생략되어 있다. (예외 : default, static 메소드 : java 1.8 버젼)
		...
		//1. 인터페이스는 객체화를 할 수 없다. 구현되지 않은 메소드가 있기 때문에
		//A a = new A();
		//B b = new B();
		...
		//4. 필드의 값을 수정할 수 없다.
		//A.a = 10;	//final이 있으므로 상수이기에 값 수정이 불가
		//B.a = 20;	//final이 있으므로 상수이기에 값 수정이 불가
		...
		//2. 추상 클래스 : 구현한 클래스를 객체화 해서 타입 지정은 가능
		C c2 = new Cc();
		...
		c2.abc();		//오버라이딩 된 메소드 출력, C 타입의 abc()를 호출, Cc 타입의 abc()
