...
interface A {
	...
	//default 메소드 : 인터페이스 내에서 구현부가 있는 메소드
		//모든 자식 클래스에서 상속되어 내려감. 새로운 기능을 추가할 때 자식 클래스에서 오류 없이 기능 추가
	default void cde () {		//default 앞에 public 생략되어 있다.
		System.out.println("블루투스 기능 추가됨");
	};
	
	//추상 메소드 : public abstract 가 생략됨.	<== 실행부가 없는 메소드 {}
		//자식 클래스에서는 선언만된 메소드를 구현해야 될 의무
...
public class Interface_Super_Use {
	...
	public static void main(String[] args) {
		...
		//a1을 B 타입으로 다운캐스팅 해야 def() 메소드를 호출할 수 있다.
