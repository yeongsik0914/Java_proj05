...
class A {	//Outer class : 필드, 메소드에 접근 제어가 붙어 있다
				//Inner class를 생성하면 Outer class의 접근 제어자 상관없이 접근
	
	public int a =3;		//다른 패키지에서 접근이 가능, 같은 패키지에서 접근이 가능
	protected int b = 4;	//다른 패키지에서 접근이 가능 (상속), 같은 패키지에서 접근이 가능
	int c= 5;				//default, 다른 패키지에서는 접근 불가능, 같은 패키지에서만 접근 가능
	private int d = 6;		//다른패키지, 같은 패키지에서도 접근 불가능, 같은 클래스에서만 접근 가능
