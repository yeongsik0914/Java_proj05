...
public class MethodOverriding_test {
	...
	public static void main(String[] args) {
		// 메소드 오버라이딩 : 상속 관게에서 부모 클래스의 메소드를 자식 클래스에서 재정의
		...
		// t1은 Animal, Tiger를 내포하고 있고, Tiger 타입으로 지정(Animal, Tiger 접근 가능)
		Tiger t1 = new Tiger();
		//부모의 필드와 메소드 호출
		t1.name = "호랑이1";
		t1.age = 5;
		t1.cry();	//Tiger 클래스에서 오버라이드 했기 때문에 어흥이 나온다.
		...
		a2.name = "호랑이2";
		a2.age = 5;
		a2.cry(); 	//오버라이딩 된 Tiger의 cry() 메소드가 호출됨.
		
		//다운 캐스팅 : a2 : animal, Tiger를 내포하고 있고 Animal 타입
		
		Tiger t2 = (Tiger) a2;	//다운 캐스팅할 때는 반드시 instanceof를 사용해야 한다.
		...
		Animal aaa = null;		//Animal 객체형 변수, 초기화 할 때 null을 사용
		for (int i=0; i<arr.length; i++) {
			aaa = arr[i];
			aaa.cry();
		}
